...
class contentMod extends commonMod
{
    ...
    public function common_list_where()
    {
		...
		$where=' where A.uid='.$this->user_info['uid'];
        return array(
            'order'=>$order,
            'where'=>$where,
            'url'=>$where_url
    ...
    public function about() {
        return $this->model->table('category_page')->field('content')->where('cid=2')->find();
    }

    public function contact() {
        return $this->model->table('category_page')->field('content')->where('cid=3')->find();
    }
