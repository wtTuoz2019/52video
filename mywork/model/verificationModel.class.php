...
class verificationModel extends commonMod {
    ...
    //验证码
    public function verify_image1($action,$html=true){
        $config=model('system')->config();
        $status=intval($config['verification_'.$action]);
        if($status){
            if($html){
                return '<input class="wenben text" placeholder="请输入用户名" name="checkcode" id="checkcode" type="text" style="width:212px !important" reg="[0-9]{4,4}" msg="验证码格式错误" > <img src="'.__APP__.'/verification/verify_img" alt="如果您无法识别验证码，请点图片更换" border="0" align="top" id="verifyImg" style="width:100px; height:50px;margin-right: -1px;" " onClick="fleshVerify()"/> ';
            }else{
                return true;
            }
        }else{
