...
class liveModel extends commonModel
{
	...
	public function get_url($id){
		$data = $this->model->table('channel')->where('id='.$id)->find();
		return $data['vMp4url'];
	}
...
public function get_livelist(){
       $timenow=time();
     $sql = "SELECT dc_content.*
            FROM dc_content
            LEFT JOIN dc_expand_content_livestream
            ON dc_expand_content_livestream.aid=dc_content.aid
            WHERE dc_expand_content_livestream.starttime<$timenow and dc_content.cid=16 and dc_expand_content_livestream.starttime+dc_expand_content_livestream.time*60>$timenow order by dc_expand_content_livestream.starttime asc ";
        $data=$this->model->query($sql);
        return $data;
    }
}
