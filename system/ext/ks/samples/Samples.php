...
$task = $client->getAdp(array("TaskID"=>'00PrvIZE90qS'));
...
$args = array(
		"Bucket"=>"urrenvideo",
		"Options"=>array(
			"prefix"=>"record/live/heims/hls/heims",
			"delimiter"=>""
			)
		);
$result=listObjects($client,$args);
$file='';

foreach($result['Contents'] as $key=>$value){
	echo date('Y-m-d H:i:s',strtotime($value['LastModified']));
	//if($key<7&&$key>0){
//	
//	$file.='&file='.base64_encode('urrenvideo:'.$value['Key']);
//	}
	}

var_dump($result['Contents']);
echo $result['Contents'][0]['Key'];die;
print_r(copyObject($client,$result['Contents'][0]['Key']));die;
 echo $commond='tag=saveas';
print_r(putAdp($client,$result['Contents'][0]['Key'],$commond));
...
function listObjects($client,$args){
...
function copyObject($client,$key){
	$args = array(
		"Bucket"=>"urrenvideo",
		"Key"=>"urren/test.m3u8",
		"CopySource"=>array(
...
function putAdp($client,$key,$commond){
	$args=array(
		"Bucket"=>"urrenvideo",
		"Key"=>$key,
		"Adp"=>array(
