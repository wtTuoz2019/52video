...
class Wechat2
{
    ...
    const CARD_MEMBERCARD_USERINFO = '/card/membercard/userinfo/get?';      //根据CardID和Code查询会员信息
    ...
    public $logcallback;
    public $set_cache_cb = null;
    public $get_cache_cb = null;
    ...
    public function __construct($options)
    {
        ...
        $this->set_cache_cb = isset($options['set_cache_cb']) ? $options['set_cache_cb'] : false;
        $this->get_cache_cb = isset($options['get_cache_cb']) ? $options['get_cache_cb'] : false;
    }
    ...
    public function valid($return = false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($this->encrypt_type == 'aes') { //aes加密
                ...
                $pc = new Prpcrypt2($this->encodingAesKey);
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function setCache($cachename, $value, $expired)
    {
        ...
        if ($this->set_cache_cb) {
            call_user_func($this->set_cache_cb, $cachename, $value, $expired);
            return true;
        }
        return false;
    }
    ...
    protected function getCache($cachename)
    {
        ...
        if ($this->get_cache_cb) {
            $ret = call_user_func($this->get_cache_cb, $cachename);
            return $ret;
        }
        return false;
    }
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '')
    {
        ...
        if ($result) {
            ...
            $this->expires_timestamp = ($json['expires_in'] ? intval($json['expires_in']) - 100 : 3600) + time();
     ...
     */
    public function getJsTicket2($appid = '', $jsapi_ticket = '')
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        if (!$appid) $appid = $this->appid;
    ...
    {
        if (!$this->jsapi_ticket && !$this->getJsTicket2($appid) || !$url) return false;
        if (!$timestamp)
            $timestamp = time();
        if (!$noncestr)
    ...
    public function getAddCardSign($card_id = '', $code = '', $timestamp = 0, $noncestr = '', $openid = '', $balance = '')
    {
        ...
        if (!$noncestr)
            $noncestr = $this->generateNonceStr();
        $arrdata = array("api_ticket" => $this->api_ticket, "card_id" => $card_id, "code" => $code, "timestamp" => $timestamp, "noncestr" => $noncestr, "openid" => $openid, "balance" => $balance);
        $sign = $this->getTicketSignature($arrdata);
        if (!$sign)
            return false;
        $signPackage = array(
    ...
    {
		var_dump($this->checkAuth());echo 111;die;
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_URL_PREFIX . self::USER_INFO_URL . 'access_token=' . $this->access_token . '&openid=' . $openid . '&lang=' . $lang);
		echo self::API_URL_PREFIX . self::USER_INFO_URL . 'access_token=' . $this->access_token . '&openid=' . $openid . '&lang=' . $lang;
		var_dump($result);die;
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
     ...
     * @return bool|mixed
     */
    public function getMemberCardUsernfo($cardId, $code)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
    ...
    public function parseSkuInfo($skuInfo)
    {
...
//////////////////////START OLD WECHAT ////////////////
    ...
    public function createNativeUrl($productid)
    {
        ...
        $nativeObj["productid"] = rawurlencode($productid);
    ...
    public function createPackage($out_trade_no, $body, $total_fee, $notify_url, $spbill_create_ip, $fee_type = 1, $bank_type = "WX", $input_charset = "UTF-8", $time_start = "", $time_expire = "", $transport_fee = "", $product_fee = "", $goods_tag = "", $attach = "")
    {
        ...
        foreach ($arrdata as $key => $value) {
            if (strlen($paramstring) == 0)
                $paramstring .= $key . "=" . rawurldecode($value);
            else
                $paramstring .= "&" . $key . "=" . rawurldecode($value);
        ...
        $stringSignTemp = $paramstring . "&key=" . rawurldecode($this->partnerkey);
        $signValue = strtoupper(md5($stringSignTemp));
        $package = $url_params . "&sign=" . rawurldecode($signValue);
        return $package;
        /*
        foreach($arrdata as $key => $value)
        {
...
class PKCS7Encoder2
{
    ...
    {
        $block_size = PKCS7Encoder2::$block_size;
        $text_length = strlen($text);
        //计算需要填充的位数
        $amount_to_pad = PKCS7Encoder2::$block_size - ($text_length % PKCS7Encoder2::$block_size);
        if ($amount_to_pad == 0) {
            $amount_to_pad = PKCS7Encoder2::block_size;
        }
        //获得补位所用的字符
    ...
    function decode($text)
    {
        ...
        if ($pad < 1 || $pad > PKCS7Encoder2::$block_size) {
            $pad = 0;
        }
...
class Prpcrypt2
{
    ...
    public function encrypt($text, $appid)
    {
        ...
        try {
            ...
            return array(ErrorCode2::$OK, base64_encode($encrypted));
        } catch (Exception $e) {
            //print $e;
            return array(ErrorCode2::$EncryptAESError, null);
        }
    }
    ...
    public function decrypt($encrypted, $appid)
    {
        ...
        try {
            ...
            return array(ErrorCode2::$DecryptAESError, null);
        }
        ...
        try {
            ...
            //print $e;
            return array(ErrorCode2::$IllegalBuffer, null);
        }
        if ($from_appid != $appid)
            return array(ErrorCode2::$ValidateAppidError, null);
