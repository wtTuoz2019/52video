 ...
 * @return string
  ...
  public function showcount($count)
 {
	  if($count)
	  {
	  	  return '<li><a  href="javascript:">共有'.$count.'条</a></li>';;
	  }
	...
	for($i=$begin;$i<$begin+$this->pageBarNum;$i++)
	{
		if($i<=$this->totalPage)
		{
			if($i!=$this->nowIndex)
				...
				$return.=$this->_getText('<li><a class="'.$nowIndex_style.'">'.$i.'</a></li>');
		}
		else
		{
			break;
 ...
 */
	public function show($url="",$total=0,$perPage=10,$pageBarNum=10,$mode=1)
	{
		...
		//翻页小于1的 不显示页码
		if($this->totalPage<=1)
			return $this->showcount($total);
		switch ($mode)
		{
			case 1:
				return $this->showcount($total).$this->firstPage().$this->prePage().$this->nowBar().$this->nextPage().$this->lastPage();
				break;
			case 2:
...
private function _requestUri()
{
  ...
  * @return string $url 
  */
	public function _getUrl($pageNum=1)
	{
	 	$url=$this->url;
		if($pageNum>1)
		{
