...
class Image {
    ...
    static public function buildImageVerify($width=48,$height=22,$randval=NULL,$verifyName='verify') {
      ...
      static public function buildImageVerify1($width=48,$height=22,$randval=NULL,$verifyName='verify') {
		if( !isset($_SESSION) ) {
			session_start();//如果没有开启，session，则开启session
		}		
		$randval =empty($randval)? ("".rand(1000,9999)):$randval;
		...
		for($i=0;$i<$length;$i++) {
			imagettftext($im,30, 0,$i*20+10,40, $stringColor, $fontfile, $randval{$i});
		}
		self::output($im,'png');
    }
