      ...
      $upload->saveRule = 'filename'; //重命名
		  ...
		  if(!$upload->upload())
        {
          ...
          if($_POST['thumb']){
		   ...
		   $image_size=getimagesize('../'.$file_url . $filetime . '/'.$info['savename']);
           $json=array('error' => 0, 'url' =>$file, 'original'=>$file_url . $filetime . '/'.$info['savename'], 'file'=>$file,'width'=>$image_size[0],'height'=>$image_size[1],'title'=>$title,'ext'=>$ext,'msg'=>'成功');
           ...
           $data['ext']=$ext;
		   $data['uid']=$this->user_info['uid']?$this->user_info['uid']:intval($_GET['uid']);
           $data['size']=$info['size'];
           $data['time']=time();
