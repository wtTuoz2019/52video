...
class  expand_modelModel extends commonModel
{
    ...
    //部分字段列表
    public function field_list_data1($mid)
    {
        $where='mid="'.$mid.'"';
        $where.=' AND fid>10';
        return $this->model->table('expand_model_field')->where($where)->order('sequence asc,fid asc')->select();
    }
    ...
    //字段列表
    public function field_list1($mid)
    {
        $list=$this->field_list_data1($mid);
        if(empty($list)){
            return;
        }
        foreach ($list as $key => $vo) {
...
//                ),
	...
	public function get_starttime($aid){
		$data = $this->model->table('expand_content_livestream')->where('aid='.$aid)->find(); 
        ...
        switch ($info['type']) {
            case '1':
                ...
                <tr>
                    ...
                    <td>
                    <input name="'.$info['field'].'" type="text"class="form-control" id="'.$info['field'].'" value="'.$info['default'].'" 
                ';
                if(!empty($info['must'])){
            ...
            case '4':
                ...
                <tr>
                    <td align="right">'.$info['name'].'</td>
                    <td>
                    <input name="'.$info['field'].'" type="text" class="form-control"  style="width:200px; float:left"  id="'.$info['field'].'" value="'.$info['default'].'" 
                ';
                if(!empty($info['must'])){
                    $html.=' reg="\S"  msg="'.$info['name'].'不能为空！" ';
                }
            ...
            case '7':
                ...
                $html.='</td>';
                $html.='<td>';
                $html.='<input name="'.$info['field'].'"  id="'.$info['field'].'" type="text"class="form-control" style="width:210px; float:left" value="'.$info['default'].'"';
                if($info['must']==1){
                    $html.=' reg="\S" msg="'.$info['name'].'不能为空" ';
                }
    ...
    public function edit_content_save($data)
    {
        ...
        if($this->model->table('expand_content_'.$model['table'])->where($condition)->count()){
			...
			$oldid = $this->model->table('content')->where($condition)->find();
			$oid = isset($oldid['oldaid']) ? $oldid['oldaid'] : $data['aid'];
			return $this->model->table('expand_content_livestream')->data($data_save)->where("aid=".$oid)->update();
        }else{
