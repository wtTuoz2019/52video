...
class menuModel extends commonModel
{
    ...
    public function admin_menu($pid=0) {
        $list=$this->model->table('admin_menu')->where('status=1 AND pid='.$pid)->order('sort desc,id asc')->select();
        $data=array();
        if(!empty($list)){
    ...
    public function menu_list($pid=0) {
        return $this->model->table('admin_menu')->where('pid='.$pid)->order('sort desc,id asc')->select();
    }
	
	 //列表
    public function model_list() {
        return $this->model->table('admin_menu')->order('sort desc,id asc')->select();
    }
	   //获取菜单树形列表
    public function menu_lists($id = 0) {
    ...
    public function info($id) {
        ...
        return $this->model->table('admin_menu')->where($where)->find();
    }
    ...
    public function edit_save($data) {
        ...
        return $this->model->table('admin_menu')->data($data)->where('id='.$data['id'])->update();
    }
    ...
    {
        $status=$this->model->table('admin_menu')->where('id='.$id.' or pid='.$id)->delete(); 
    ...
    public function gentree($data,$class_power=array(),$keep=true){
        ...
        if(!empty($data)){
            foreach ($data as $key=>$value) {
                $tree[$key]=$value;
                if(in_array($value['cid'],(array)$class_power)||$keep){
