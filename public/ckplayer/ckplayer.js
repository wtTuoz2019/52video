...
//    cpt += 'share.swf,1,1,-180,-100,3,0|'; //分享插件
	...
	if(statusarray.time>0&&statusarray.time<starttime&&starttime!=0&&startflag){
		
		startflag=false;
		CKobject.getObjectById('ckplayer_a1').videoSeek(starttime);
		}
	...
	function getprogress(){
		...
		$('#start').text('开始：'+formatDate(a));
		$('#start2').text('开始：'+formatDate(a));
	...
	function getprogressend(){
		...
		var totaltime=parseInt(statusarray.totalTime);
		
		if(typeof(statusarray.totaltime) != "undefined"){
			
			totaltime=parseInt(statusarray.totaltime);
			}
		var left= parseInt($('#progressinput').val())/totaltime;
		var width=(parseInt($('#progressendinput').val())-parseInt($('#progressinput').val()))/totaltime;
		...
		width=(width*100)+'%';
		$('#progress1').css({'margin-left':left,'width':width});
		$('#progress2').css({'margin-left':left,'width':width});
		
		$('#end').text('结束：'+formatDate(a));
		$('#end2').text('结束：'+formatDate(a));
...
if (i<10) 
  ...
  return i
	...
	//视频观看
			function nowpagetoplayseek(e){
				$('#screen').css('display','block');
				starttime=parseFloat($(e).attr('data-time'));
		//	$('body,html').animate({
//						scrollTop: 0 ,
//					 	}, 300
//					);
			...
			startflag=flag=true;
				var w = "100%";//视频宽度
  var h = "200";//视频高度
  var url = $(e).attr('videourl');//视频地址
  var image = "{$info.image}";//封面图片
  var flashvars={f:'/public/plays/m3u8.swf',a:url,c:0,s:4,i:image,lv:0,p:1};
  var params={bgcolor:'#FFF',allowFullScreen:true,allowScriptAccess:'always',wmode:'transparent'};
  var video=[url];
CKobject.embed('/public/ckplayer/ckplayer.swf','a1','ckplayer_a1',w,h,false,flashvars,video,params);
...
cpt_definition:'0x656565,0x2c2c2c,80,20,0xFFFFFF,0x00b4ff,10,10,1,3,自动,12,MicrosoftYaHei|微软雅黑,0x505050,10,80,20,15,0,10,15,0x656565,0x2c2c2c,80,20,0xFFFFFF,0x00b4ff,10,10,1,3,12,MicrosoftYaHei|微软雅黑,50,0xb7b7b7,40,1,10,10',
		...
		*/
		...
		/*
			跳过广告和播放结束时调用的js函数
		*/
        myweb: escape('0970ce0f49a6123b57cbdd09dfbd5eed,shanyueyun,http://live.shanyueyun.com,v1.0'),
        /*
		------------------------------------------------------------------------------------------------------------------
		...
		*/
        cpt_share: '/public/ckplayer/share.xml',
        /*
		分享插件调用的配置文件地址
		调用插件开始
		*/
...
(function() {
    var CKobject = {
        ...
        showHtml5: function() {
			...
			var v = '<video webkit-playsinline=true playsinline=true x-webkit-airplay=false '+j+r+' id="' + p + '" width="' + w + '" height="' + h + '"' + C.getParams() + '>' + C.getVideo() + '</video>';
            C._K_(c).innerHTML = v; 
