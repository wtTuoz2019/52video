...
class labelMod extends commonMod {
	...
	public function getlist($data) {
		...
		switch ($table) {
            ...
            case 'tags':
                ...
                break;
			case 'teacher':
                return $this->get_teacher($where,$label);
                break;
			default:
    ...
    public function get_content($where,$data) {
        ...
        if(!empty($data['related'])){
            ...
            if(!empty($tags_aid)){
                ...
                if(preg_match("/[^\d-., ]/",$data['related'])){
                    ...
                    $related.=' AND A.aid<>'.$data['related_id'];
                }
            }else{
                return ;
            }
	 ...
	 //其他表获取
    public function get_teacher($where,$data)
    {	
        $condition=@substr($where['where'], 4);
		if($_SESSION['sid']){
			if($condition)
			$condition.=' AND sid='.$_SESSION['sid'];
			else
    ...
    public function get_aurl($aid,$app='')
    {
			...
			$url_content='content/index?aid='.$aid;
			if($this->config['token']){
				  $url_content.='&token='.$this->config['token'];
				}
